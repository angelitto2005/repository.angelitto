...
class rarbg(Torrent):
    def __init__(self):
        ...
        self.appid = 'plugin.video.mrsplite'
        self.thumb = os.path.join(media, 'rarbg.png')
        self.name = 'Rarbg'
        self.search_url = '%s?mode=search&search_string=%s%s&limit=50' % (self.base_url, '%s&app_id=plugin.video.mrsplite&format=json_extended&ranked=0', '&sort=seeders')
    ...
    def get_token(self):
        try: 
            token = fetchData('%s?get_token=get_token&app_id=plugin.video.mrsplite' % self.base_url, rtype='json', api=1)["token"]
            return token
        except: pass
