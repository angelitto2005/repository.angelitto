...
from resources.functions import log,cutFileNames,unescape,ensure_str,get_ids_video,isSubtitle,showMessage,is_writable, requests, get_ip, check_torrent2http, pbar, ROOT, __settings__
...
class MRPlayer(xbmc.Player):
    ...
    def start(self,uri,path='',cid=None,params={},files=None,browse=False,download=False):
        ...
        if not re.match(r"^magnet\:.+", self.torrentUrl) and not self.torrentUrl.startswith('file:'):
            self.savetorrent()
        self.setup_engine()
    ...
    def torrent_exists(self, play=False, seed=False):
        if not self.browse:
            ...
            hashtorrent = None
            if not re.match(r"^magnet\:.+$", self.torrentUrl):
                ...
                hashtorrent = re.findall(r'btih\:([a-fA-F0-9]+)', self.torrentUrl)[0]
    ...
    def seed(self):
        if self.seedingtransmission:
            ...
            if not re.match(r"^magnet\:.+$", self.torrentUrl):
                ...
                hashtorrent = re.findall(r'btih\:(.+?)&', self.torrentUrl)[0]
            numberfiles = []
            i = -1
            for x in files:
                i += 1
            ...
            if lists not in [False, None]:
                ...
                if tid:
                    ...
                    if not setip in localhosts:
                        if not re.match(r"^magnet\:.+$", fileforseed):
