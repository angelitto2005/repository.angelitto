...
def search():
    ...
    # --- FALLBACK OPENSUBTITLES (Dacă RO/EN sunt goale) ---
    if not os_results:
    ...
    # Adăugăm rezultatele OpenSubs în lista finală
    all_results.extend(os_results)

    # --- 2. CĂUTARE KOOFR (DOAR LIMBA ALEASĂ / RO) ---
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            f_target = f".{l_code}." # Ex: ".ro."
            
            for f in files:
                f_clean = f.strip('/')
                # FILTRARE: Arată doar dacă e subtitrare ȘI conține limba aleasă (ex: .ro.)
                if f_clean.lower().endswith(('.srt', '.sub')) and f_target in f_clean.lower():
                    ...
                    all_results.append({
                        ...
                        'vtt_url': '', 
                        'l_code': l_code, # Forțăm l_code-ul să fie egal cu limba aleasă -> NU TRADUCE
    ...
    # Asigură-te că rank_colors este definit înainte de buclă
    ...
    for res in all_results:
        ...
        # Parametri pentru DOWNLOAD (Inclusiv datele pentru LOGGER)
        # Atenție: imdb_id, title_raw, season, episode trebuie să existe în funcția search
        d_params = {
            'action': 'download', 
            'url': res['url'], 
        ...
        # Folosim urllib.parse.urlencode pentru a evita NameError
...
def download(params):
    ...
    # Adăugăm calea către folderul 'lib' pentru a putea importa uploader.py
    addon_path = xbmcvfs.translatePath(__addon__.getAddonInfo('path'))
    # Modifică linia de mai jos dacă folderul lib este în altă parte (ex: resources/lib)
    ...
    try:
        ...
        if content:
            ...
            # ÎNCHIDERE FEREASTRĂ ȘI SETARE SUBTITRARE
            ...
            # --- LOGICĂ ROBOT ȘI STATISTICI KOOFR ---
            langs = ["ro", "en", "es", "fr", "de", "it", "hu", "pt", "ru", "tr", "bg", "el", "pl", "cs", "nl"]
            ...
            # TRIMITEM LOG-UL CĂTRE uploader.send_log
            try:
                import uploader
                log_info = {
