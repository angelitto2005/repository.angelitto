...
# 1. Definim calea corectă către resurse
lib_path = xbmcvfs.translatePath(os.path.join(__addon__.getAddonInfo('path'), 'resources', 'lib'))

# 2. Adăugăm calea la începutul listei Python pentru prioritate maximă
if lib_path not in sys.path:
    sys.path.insert(0, lib_path)

# 3. Importăm roboții (dacă unul eșuează, ceilalți se încarcă în continuare)
try: import robot
...
def download(params):
    ...
    try:
        ...
        if content:
            ...
            # --- PREGĂTIRE ROBOT (ÎNAINTE de endOfDirectory) ---
            robot_activat = __addon__.getSettingBool('robot_activat')
            ...
            if l_code != chosen_lang and robot_activat:
                is_tr = True
                try:
                    # Importăm roboții direct aici pentru a fi siguri că sunt disponibili în funcție
