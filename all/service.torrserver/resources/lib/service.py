...
if IS_ANDROID:
    ...
    # Detectare userspace 32-bit pe kernel 64-bit (ex: Vero 4K)
    if _arch in ('aarch64', 'arm64', 'armv8l', 'armv8'):
        try:
            long_bit = subprocess.check_output(['getconf', 'LONG_BIT'], timeout=2).decode().strip()
            if long_bit == '32':
                _arch = 'armv7l'
        except Exception:
            pass
    # Normalizare denumiri arhitectura
    if _arch.startswith('armv8') or _arch.startswith('aarch64'):
        _arch = 'aarch64'
    elif _arch.startswith('armv7'):
        _arch = 'armv7l'
    elif _arch.startswith('armv6'):
        _arch = 'armv6l'
    elif _arch.startswith('armv5'):
        _arch = 'armv5l'
    elif _arch.startswith('x86_64') or _arch == 'amd64':
        _arch = 'x86_64'
    elif _arch.startswith('i686') or _arch.startswith('i386'):
        _arch = 'i386'
    xbmc.log(f"[service.torrserver] Arhitectura detectata: {_arch}", xbmc.LOGINFO)

IS_ARM = _arch in ('aarch64', 'armv8', 'arm64', 'armv8l', 'armv7l', 'armv7', 'armv6l', 'armv5', 'armv5l')

# Mapare arhitectura -> nume binar
_LINUX_ARCH_MAP = {
    'aarch64': 'linux-arm64',
    'arm64': 'linux-arm64',
    'armv8': 'linux-arm64',
...
if IS_WINDOWS:
    ...
    BIN_NAME = f'TorrServer-{_ANDROID_ARCH_MAP.get(_arch, "android-arm64")}'
else:
    BIN_NAME = f'TorrServer-{_LINUX_ARCH_MAP.get(_arch, "linux-arm64")}'
...
class TorrServerService(xbmc.Monitor):
    ...
    def read_settings(self):
        ...
        self.force_arch           = a.getSetting('force_arch')
        self.auto_cleanup_hours   = s2i(a.getSetting('auto_cleanup_hours'), 0)
        ...
        xbmc.log(f"[{ADDON_ID}] Arhitectura detectata: {_arch} | Force arch: {self.force_arch} | Binar: {self._get_bin_name()}", xbmc.LOGINFO)

    def _get_bin_name(self):
        if self.force_arch and self.force_arch != 'auto':
            suffix = self.force_arch
            if IS_WINDOWS and not suffix.endswith('.exe'):
                suffix += '.exe'
            return f'TorrServer-{suffix}'
        return BIN_NAME

    def _get_binary_path(self):
        name = self._get_bin_name()
        if IS_ANDROID:
            return os.path.join(ADDON_DATA, 'bin', name)
        return os.path.join(ADDON_PATH, 'resources', 'bin', name)
    ...
    def _torrents_request(self, payload=None, timeout=5):
        if payload:
            data = json.dumps(payload).encode()
            req = urllib.request.Request(f"http://127.0.0.1:{self.port}/torrents", data=data, method="POST")
            req.add_header("Content-Type", "application/json")
        else:
            req = urllib.request.Request(f"http://127.0.0.1:{self.port}/torrents", method="GET")
        for k, v in self._auth_header().items():
            req.add_header(k, v)
        with urllib.request.urlopen(req, timeout=timeout) as r:
            return json.loads(r.read().decode())

    def _cleanup_old_torrents(self):
        hours = self.auto_cleanup_hours
        if hours <= 0:
            return
        try:
            torrents = self._torrents_request()
            ...
            for t in torrents:
                ...
                if not t_hash:
                    continue
                state = t.get('state', 0)
                stop_date = t.get('stop_date', 0)
                if state >= 2 and stop_date > 0 and stop_date < cutoff:
                    self._torrents_request({"action": "remove", "hash": t_hash}, timeout=5)
                    xbmc.log(f"[{ADDON_ID}] Cleanup: eliminat torrent {t.get('title', t_hash)} (oprit de {int((now-stop_date)/3600)}h)", xbmc.LOGINFO)
                    removed += 1
            if removed:
                xbmc.log(f"[{ADDON_ID}] Cleanup: {removed} torente eliminate", xbmc.LOGINFO)
        except Exception as e:
            xbmc.log(f"[{ADDON_ID}] Cleanup error: {str(e)}", xbmc.LOGDEBUG)
    ...
    def download_binary(self):
        ...
        try:
            ...
            xbmc.log(f"[{ADDON_ID}] Descarcare completa: {bin_path}", xbmc.LOGINFO)
            return True
        except Exception as e:
            xbmc.log(f"[{ADDON_ID}] Eroare la descarcare: {str(e)}", xbmc.LOGERROR)
            tmp_path = self._get_binary_path() + '.tmp'
    ...
    def _check_update_on_start(self):
        ...
        try:
            ...
            if self.auto_update:
                ...
                if not confirmed:
                    return
                xbmc.log(f"[{ADDON_ID}] Auto-update: instalez {latest_ver}...", xbmc.LOGINFO)
                self.stop_torrserver()
                time.sleep(1)
                if self.download_binary():
                    if not IS_WINDOWS:
                        try:
                            os.chmod(self._get_binary_path(), 0o755)
                        except Exception:
                            pass
                    self.start_torrserver()
                    xbmcgui.Dialog().notification('TorrServer', f'Updated to {latest_ver}!', xbmcgui.NOTIFICATION_INFO, 4000)
                else:
                    xbmc.log(f"[{ADDON_ID}] Auto-update download esuat", xbmc.LOGERROR)
                    self.start_torrserver()
            elif self.notify_update_on_start:
    ...
    def rollback(self):
        bak_path = self._get_binary_path() + '.bak'
        ...
        try:
            ...
            time.sleep(1)
            bin_path = self._get_binary_path()
            shutil.copy2(bak_path, bin_path)
            if not IS_WINDOWS:
                try:
                    os.chmod(bin_path, 0o755)
                except Exception as e:
    ...
    def run(self):
        ...
        while not self.abortRequested():
            if self.waitForAbort(5):
                break
            if self.auto_cleanup_hours > 0:
                cleanup_interval += 5
                if cleanup_interval >= 600:
                    cleanup_interval = 0
                    threading.Thread(target=self._cleanup_old_torrents, daemon=True).start()
        self.stop_torrserver()
